...
$publicTitle='Start Your CuePOS Trial | Snooker Club Software';
$publicDescription='Create a private CuePOS workspace for your snooker club and begin a 14-day free trial.';
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $planSlug=preg_replace('/[^a-z0-9_-]/','',strtolower($_POST['plan_slug']??'starter'));
    if(!$owner||!$email||!$club||strlen($password)<10){
        ...
        try{
            ...
            if(!$plan)throw new RuntimeException('That plan is not available.');
            ...
            $q=$db->prepare("INSERT INTO clubs(name,slug,address,phone,tv_token,country_code,timezone,locale,currency_code,currency_symbol,tenant_status,trial_ends_at) VALUES(?,?,?,?,?,?,?,?,?,?, 'trial',DATE_ADD(NOW(),INTERVAL 14 DAY))");
            $q->execute([$club,$slug,null,null,$token,$country?:null,$timezone,'en',$currency[0],$currency[1]]);
            ...
            $db->prepare("INSERT INTO subscriptions(club_id,plan_id,status,billing_interval,trial_ends_at,starts_at) VALUES(?,?, 'trial','monthly',DATE_ADD(NOW(),INTERVAL 14 DAY),NOW())")->execute([$clubId,$plan['id']]);
            ...
            foreach(['onboarding_complete'=>'0','subscription_plan'=>$plan['slug'],'trial_notice'=>'1'] as $k=>$v)$settings->execute([$clubId,$k,$v]);
...
<main class="relative overflow-hidden">
    ...
    <div class="max-w-6xl mx-auto px-5 py-14 lg:py-20 grid lg:grid-cols-[.9fr_1.1fr] gap-10 xl:gap-16 items-start">
        <section class="lg:pt-8">
            <span class="eyebrow"><span class="w-2 h-2 bg-cue rounded-full"></span>14-day free trial</span>
        ...
        <section class="form-shell">
            <div class="flex items-start justify-between gap-4 pb-6 border-b border-slate-200">
                <div><div class="flex items-center gap-3"><span class="form-step">1</span><span class="text-xs font-extrabold uppercase tracking-[.13em] text-cue-deep">Create your workspace</span></div><h2 class="font-display text-3xl font-bold tracking-tight mt-4">Start your CuePOS trial</h2><p class="text-sm text-slate-500 leading-6 mt-2">Use your business details. You can invite your team after setup.</p></div>
            ...
            <form method="post" class="mt-6">
                ...
                <div class="grid sm:grid-cols-2 gap-5">
                    ...
                    <label class="field sm:col-span-2"><span class="field-label">Plan after your trial <span class="field-hint">Change anytime</span></span><select class="field-control field-select" name="plan_slug"><?php foreach($plans as $p):?><option <?=($selectedPlan===$p['slug']?'selected':'')?> value="<?=sanitize($p['slug'])?>"><?=sanitize($p['name'])?><?= $p['price_monthly']?' — $'.number_format($p['price_monthly'],0).'/month':''?></option><?php endforeach?></select></label>
                </div>
                <button class="btn-public btn-public-primary w-full mt-7" type="submit">Create my secure workspace <i class="fa-solid fa-arrow-right"></i></button>
                <div class="form-trust"><i class="fa-solid fa-lock"></i><span>By continuing, you agree to the CuePOS <a class="marketing-link" href="<?=url('terms.php')?>">Terms</a> and <a class="marketing-link" href="<?=url('privacy.php')?>">Privacy Policy</a>. No payment information is collected during the trial.</span></div>
            </form>
        </section>
    </div>
</main>
