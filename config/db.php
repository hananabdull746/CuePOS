...
function publicPlans(): array { try { $q=getDB()->query('SELECT * FROM plans WHERE is_public=1 ORDER BY sort_order,id');$plans=$q->fetchAll();if($plans)return $plans; } catch(Throwable $e) {} return [['slug'=>'free','name'=>'Free Plan','tagline'=>'Start with one table','description'=>'One table and basic billing to get your club started.','price_monthly'=>0,'price_yearly'=>0,'currency_code'=>'PKR','table_limit'=>1,'staff_limit'=>1],['slug'=>'basic','name'=>'Basic Plan','tagline'=>'Built for growing clubs','description'=>'Up to five tables with reliable daily billing and core club operations.','price_monthly'=>499,'price_yearly'=>4990,'currency_code'=>'PKR','table_limit'=>5,'staff_limit'=>5],['slug'=>'pro','name'=>'Pro Plan','tagline'=>'Run the complete club','description'=>'Unlimited tables, café operations, reports and full club management.','price_monthly'=>999,'price_yearly'=>9990,'currency_code'=>'PKR','table_limit'=>9999,'staff_limit'=>25]]; }
