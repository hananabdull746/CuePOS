...
function publicPlans(): array { try { $q=getDB()->query('SELECT * FROM plans WHERE is_public=1 ORDER BY sort_order,id');$plans=$q->fetchAll();if($plans)return $plans; } catch(Throwable $e) {} return [['slug'=>'free','name'=>'Free Plan','tagline'=>'Start with one table','description'=>'One table and basic billing to get your club started.','price_monthly'=>0,'price_yearly'=>0,'currency_code'=>'PKR','table_limit'=>1,'staff_limit'=>1],['slug'=>'pro','name'=>'Pro Plan','tagline'=>'Pay per table as your club grows','description'=>'Rs. 449 per table per month for up to 10 tables.','price_monthly'=>449,'price_yearly'=>0,'currency_code'=>'PKR','table_limit'=>10,'staff_limit'=>10],['slug'=>'premium','name'=>'Premium Plan','tagline'=>'For clubs with more than 10 tables','description'=>'Rs. 400 per table per month with café, reports and full club management.','price_monthly'=>400,'price_yearly'=>0,'currency_code'=>'PKR','table_limit'=>9999,'staff_limit'=>25]]; }
